<script lang="ts" setup>
  ...
  const { video_playing } = storeToRefs(videoStore)
  ...
  onMounted(() => {
    if (video.value) {
      video.value.volume = volume.value || 1
    }
